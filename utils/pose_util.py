...
def calc_vos_safe_fc(poses):
    """ 计算一组连续位姿（poses）之间的 视觉里程计  把连续帧位姿列表转成帧间相对位姿矩阵，用于训练或特征增强
  ...
  """
    ...
    return vos #！ 最终输出 [N x TC2 x 7]，对应每个样本的所有帧对 VO。
...
def process_poses(poses_in, mean_t, std_t, align_R, align_t, align_s):
    # 对位姿数据进行归一化和坐标系对齐。它将输入的原始位姿数据（通常是 3x4 或 4x4 矩阵的展平形式）转换为一个更适合模型训练的格式
    ...
    for i in range(len(poses_out)):
        ...
        q = qlog(q)
        poses_out[i, 3:] = q #! 存储到3-6列
        t = poses_out[i, :3] - align_t #! 对位姿的平移分量进行平移对齐
...
def filter_overflow_ts(filename, ts_raw):
    # 根据INS真值数据的时间戳【范围】来过滤LiDAR点云的时间戳，确保数据在后续处理时能够正确地进行时间同步
    ...
    ts_filted = [t for t in ts_raw if min_pose_timestamps < t < max_pose_timestamps] # 遍历原始LiDAR时间戳列表 ts_raw，只保留那些位于真值时间戳范围之内的时间戳。
