...
if __name__ == '__main__':
    ...
    parser.add_argument('--velodyne-dir', type=str, default='/ava16t/lw/Data/XMU/XAC')
    ...
    # file_list = ['2019-01-18-15-20-12-radar-oxford-10k', '2019-01-14-14-48-55-radar-oxford-10k', '2019-01-14-12-05-52-radar-oxford-10k',\
    #              '2019-01-11-14-02-26-radar-oxford-10k', '2019-01-10-11-46-21-radar-oxford-10k', '2019-01-15-13-06-37-radar-oxford-10k',\
    #              '2019-01-17-14-03-00-radar-oxford-10k', '2019-01-18-14-14-42-radar-oxford-10k', '2019-01-17-13-26-39-radar-oxford-10k']
    # file_list = ['2012-02-18', '2012-05-11', '2012-02-12', '2012-02-19']
    # file_list = ['2012-03-31', '2012-05-26']
    file_list = ['202311281']
    for file in file_list:
        input_path = os.path.join(args.velodyne_dir, file, 'velodyne_left')
        input_point_clouds = sorted(os.listdir(input_path))
        # input_point_clouds = ['/ava16t/lw/Data/XMU/XAC/202311281/velodyne_left/1701145268092683.bin']
        for point_cloud_name in input_point_clouds:
            ...
            # pc = pc.transpose()
            # pc = np.concatenate((pc[:, :2], (-1 * pc[:, 2]).reshape(-1, 1), pc[:, 3].reshape(-1, 1)), axis=1)
            # pc[:, 2] = -1 * pc[:, 2]
            # outlier remove
            # pc = outlier_remove(pc)
            # print(pc.shape)
            ...
            ground_list = np.concatenate((ground_list, 2 * np.ones((len(ground_list), 1))), axis=1)
            ...
            tree_list = np.array(tree_list).reshape(-1, 4)
            tree_list = np.concatenate((tree_list, 3 * np.ones((len(tree_list), 1))), axis=1)
            trees = tree_list.astype(np.float32)
            ...
            results = np.concatenate((ground_list, plane_list, tree_list), axis=0)
            # np.savetxt(point_cloud_name[:-4] + 'segmented.txt', results, fmt='%.6f')
            results = results.astype(np.float32)
            import ipdb
            ipdb.set_trace()
            results.tofile(os.path.join(input_path[:-13]+'SPVNAS_tmpvelodyne_left_plane_segmented', point_cloud_name))

            # no_others = np.concatenate((ground_list, plane_list), axis=0)
            # no_others = no_others.astype(np.float32)
            # no_others.tofile(os.path.join(input_path[:-13]+'SPVNAS_velodyne_left_no_other', point_cloud_name))
