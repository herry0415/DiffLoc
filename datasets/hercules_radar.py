...
class Hercules_Radar(data.Dataset):
    def __init__(self, config, split='train'):
        ...
        #     if self.is_train: #todo 需要提前分割好数据集
        #         self.pcs.extend([osp.join(seq_dir, 'Radar/SPVNAS_multi_frame7_plane_segmented', '{:d}.bin'.format(t)) for t in ts[seq]])
        #         # self.pcs.extend(os.path.join(seq_dir, 'Radar/multi_frame_w7', str(t)+'_multi_w7' + '.bin') for t in ts[seq])
        #         vo_stats[seq] = {'R': np.eye(3), 't': np.zeros(3), 's': 1}
        #     else:
        #         self.pcs.extend(os.path.join(seq_dir, 'Radar/multi_frame_w7', str(t) + '.bin') for t in ts[seq])
        #         vo_stats[seq] = {'R': np.eye(3), 't': np.zeros(3), 's': 1}
    ...
    def __getitem__(self, idx_N):
        ...
        #! radar训练的时候 不需要进行语义风格 需要把loss里面掩码部分给注释掉
        pointcloud = np.fromfile(scan_path, dtype=np.float32).reshape(-1, 8)   
        pointcloud = np.concatenate((pointcloud[:, :3], pointcloud[:, 5:6]), axis=1) #! radar的第6个RCS是强度 [N, 4]
        pointcloud = np.concatenate((pointcloud, np.zeros((len(pointcloud), 1))), axis=1) #! 语义掩码 [N, 5]
        
        # if self.is_train:
        #     #! 训练用 将倒数第二级文件夹 multi_frame_w7 改为 SPVNAS_multi_frame7_plane_segmented
        #     parts = scan_path.split(os.sep)
        #     if len(parts) >= 2:
        #         parts[-2] = 'SPVNAS_multi_frame7_plane_segmented'
        #     scan_path = os.sep.join(parts)
        #     # generate by SPVCNN, (x, y, z, intensity, static objects mask)
        #     pointcloud = np.fromfile(scan_path, dtype=np.float32).reshape(-1, 5)
        # else:
        #     #fill with zeros 测试用原文件
        #     pointcloud = np.fromfile(scan_path, dtype=np.float32).reshape(-1, 8)   
        #     pointcloud = np.concatenate((pointcloud[:, :3], pointcloud[:, 5:6]), axis=1) #! radar的第6个RCS是强度 [N, 4]
        ...
        , None] #! 归一化
        proj_feature_tensor = proj_feature_tensor * proj_mask_tensor.unsqueeze(0).float() #! 用的是mask tensor 
