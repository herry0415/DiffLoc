...
def interpolate_ins_poses(ins_path, pose_timestamps, origin_timestamp, use_rtk=False): #！ 从 INS（惯性导航系统）数据中读取位姿，并为指定的时间戳列表生成插值后的位姿
    ...
    with open(ins_path) as ins_file:
        ...
        ins_timestamps = [0]
        abs_poses = [ml.identity(4)] # 存储对应的 4x4 绝对位姿矩阵。初始元素 ml.identity(4) 是一个单位矩阵

        upper_timestamp = max(max(pose_timestamps), origin_timestamp) # 计算需要处理的数据的最高时间戳。这可以避免处理文件中所有数据

        for row in ins_reader:
            timestamp = int(row[0])
            ins_timestamps.append(timestamp)

            utm = row[5:8] if not use_rtk else row[4:7]  #todo  索引需要更改
            rpy = row[-3:] if not use_rtk else row[11:14] #todo  索引需要更改
    ...
    ins_timestamps = ins_timestamps[1:] # 移除占位符: 移除在函数开头为了初始化而添加的第一个元素（0 和单位矩阵）
...
def interpolate_poses(pose_timestamps, abs_poses, requested_timestamps, origin_timestamp):
    ...
    for i, pose in enumerate(abs_poses):
        ...
        abs_quaternions[:, i] = so3_to_quaternion(pose[0:3, 0:3 ]) # 四元数在处理三维旋转插值时比欧拉角或旋转矩阵更稳定
        abs_positions[:, i] = np.ravel(pose[0:3, 3]) # 提取每个位姿的平移向量，将其展平为一维数组
    ...
    return poses_out #! 4x4 的 NumPy 矩阵。这个矩阵代表一个 SE3 齐次变换  这些位姿是相对于 origin_timestamp 处的位姿的。 
